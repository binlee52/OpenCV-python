...
def affine(img):
    ...
    dst = cv2.warpAffine(img, aff_mat, img.shape[1::-1], cv2.INTER_LINEAR)
    draw_rect("image", np.copy(img), pts1)
...
image = cv2.imread("images/aircraft.jpg", 1)
if image is None:
